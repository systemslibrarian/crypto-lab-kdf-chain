 ...
 *    files the node under `incomplete`. `select.input-field` paints its caret
 *    with two `linear-gradient` layers, and nearly every accented surface in
 *    this lab is a `color-mix(...)` wash over the panel or the page
 *    (`.info-box`, `.decision-result`, `.chain-root`, `.chain-leaf`,
 *    `.hkdf-prk`, `.hkdf-block`, `.pb-acc`, `.attack-out`), so their text
 *    composites onto whatever is behind rather than onto a flat colour axe can
 *    read.
 ...
 *    `opacity: .85`, and `.hkdf-block-enter` / `.pb-link-enter` start at
 *    `opacity: 0` until the entrance transition clears them — the class of
 *    defect the gate's `expectNotBlank` exists to catch.
 *
 ...
 * rather than judged at its worst stop — see `sampleLayer`.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. `.table-scroll`
 *    is an `overflow-x: auto` scroller around a `.comparison-table` carrying
 *    `min-width: 600px`, so at 380px most of its columns are clipped away; the
 *    `.output-hex` and `.attack-out` panes clip the same way vertically.
 ...
 *    draws no SVG text today — its only SVG is the shared header's icon set,
 *    which carries no `<text>` — but the rule is kept because it is cheap and
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * "why it matters" wash is `color-mix(in oklab, ...)`; every panel tint is
     * `color-mix(in srgb, ...)`), and Chromium reports those to
     ...
     * fabricates failures (a dark-theme hex readout read as sitting on white)
     * and, far worse, could hide a real one. The 2D canvas is the browser's own colour
     ...
     *
     * An older form of this helper judged every gradient at its worst *stop*,
     * assuming that stop covered the text wherever the text sat. That is right
     * for a gradient that spans its element and wrong for a small, positioned
     * one. This lab's `select.input-field` is exactly the wrong case: its caret
     * is two 5x5px `linear-gradient` layers pinned near the right edge with
     * `background-position`, each running `transparent 50%` to `--text-muted
     * 50%`. Judged at its worst stop, every `<select>` would report its own
     * option text sitting on a muted grey it never touches — one lab's footer
     * links read 3.56:1 that way where they actually render ~4.9:1. So each
     ...
     *
     * This page renders up to 128 memory cells, 48 compute cores and 48 stalled
     * cores, plus a text node per table cell across a seven-column comparison
     * table, and every one of them walks the same handful of ancestors. Without
     ...
     * is exactly the classic recipe (`clip: rect(0,0,0,0)`), and it carries the
     * eight `aria-live` announcement regions every panel writes its result
     * into — so without this guard every derivation, benchmark and vector run
     * would add a fabricated finding.  Deliberately narrow — only a ZERO-area
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page's disclosures are the three
      // `details.assumptions` notes under each attacker-cost readout, whose
      // open state is a real state the gate scans explicitly.
     ...
     * that covers the `.chain-arrow-glyph` "↓", the `.guide-num` step numerals
     * whose meaning is already in the adjacent title, and the `.hkdf-op`
     * "HMAC-SHA-256" pill duplicated in the surrounding prose.
     */
